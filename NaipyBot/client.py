...
@bot.tree.context_menu(name="다국어로 번역")
async def dual(interactions: discord.Interaction, word: discord.Message):
    try:
        target = ["ko", "en", "ja"]
        detect = await naipy.translation.detect(word.content)
        target.remove(detect.langCode) # 언어인식 후 타겟리스트에서 제거
        data: model.N2mtNaipy = await naipy.translation.dual_translation(
            text=word.content, target=target
        )  # Naipy 데이터 가져옴
    except HTTPException:
        return await interactions.response.send_message(
            "지원하지 않는 메세지형식입니다.", ephemeral=True
    ...
    embed: Embed = Embed()
    embed.add_field(
        name=f"{data[0].srcLangType} → {data[0].tarLangType}",
        value=f"`{word.content}` → `{data[0].translatedText}`",
        inline=False
    )
    embed.add_field(
        name=f"{data[1].srcLangType} → {data[1].tarLangType}",
        value=f"`{word.content}` → `{data[1].translatedText}`",
        inline=False
    )
    return await interactions.response.send_message(embed=embed, ephemeral=True)
